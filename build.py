...
def build(_: Any):  # one argument is passed by poetry but we don't need it
    ...
    if sys.platform == "win32":
        ...
        run(
            'export LD_LIBRARY_PATH="/opt/glibc-2.34/lib:$LD_LIBRARY_PATH" && export PATH="/opt/glibc-2.34/bin:$PATH" && export PATH="$HOME/.local/share/fnm:$PATH" && eval "$(fnm env --use-on-cd)" && node ./node_modules/.bin/tree-sitter generate'
        )
        run(
            'export LD_LIBRARY_PATH="/opt/glibc-2.34/lib:$LD_LIBRARY_PATH" && export PATH="/opt/glibc-2.34/bin:$PATH" && export PATH="$HOME/.local/share/fnm:$PATH" && eval "$(fnm env --use-on-cd)" && node ./node_modules/.bin/tree-sitter build -o build/rsm.so'
        )
    else:
